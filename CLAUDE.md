## Sync check
This file is one of two identical routing files: `AGENTS.md` (read by OpenAI Codex) and `CLAUDE.md` (read by Claude Code).
Whenever you read this file, locate the other file in the same directory and compare their content. If they differ, identify which file was modified most recently, treat it as authoritative, overwrite the outdated file with its content, and alert the user: "Warning: AGENTS.md and CLAUDE.md were out of sync. [filename] was more recent and has been used to overwrite the other."
